...
与它对话，它可以搜索网页、管理待办事项、记笔记、设置提醒、总结邮件等等。它会记住你跨会话的对话内容，并且可以通过新增工具或接入
...
- **提醒** — 用自然语言设置提醒（"2小时后提醒我给妈妈打电话"），支持周期性提醒。
- **邮件监控** — 后台任务通过 IMAP 监控 Gmail、Outlook 和 QQ 邮箱，用 AI 总结新邮件并推送通知到 Discord。
- **MCP 集成** — 连接任何 [MCP](https://modelcontextprotocol.io/) 服务器（GitHub、Notion、文件系统、数据库等），无需写代码即可接入数百种工具。
- **通知交互** — 回复机器人的任何消息（邮件通知、之前的回答），AI 会自动看到原始内容作为上下文。
- **热更新配置** — 任务调度、提示词、启用/禁用状态存储在 JSON 文件中，运行时修改即时生效，无需重启。
...
| `local_files`  | 浏览本地文件（只读）               | `LOCAL_FILES_ALLOWED_PATHS` |
| `memo`         | 保存、搜索、列出、删除个人笔记          | —                           |
| `reminder`     | 设置定时和周期性提醒               | —                           |
| `task`         | 待办事项管理，支持优先级和截止日期        | —                           |
| `mcp_server`   | 管理 MCP 服务器连接             | —                           |
...
├── apps/bot/                    # 主应用
│   ├── main.py                  # 入口
│   ├── config/                  # 配置、提示词、任务配置、MCP 配置
│   ├── core/                    # 编排层（启动、工具调用循环、加载器）
